...
void Renderer::DrawFS()
{
	...
	int u_SamplerTotalNum = glGetUniformLocation(shader, "u_TotalNumTexture");
	glUniform1i(u_SamplerNum, 11);
	int u_SamplerNumber = glGetUniformLocation(shader, "u_Number");
	glUniform1i(u_SamplerNum, number);
...
void Renderer::DrawBloomParticle()
{
	...
	for (int i = 0; i < 20; ++i) {
		...
		DrawTexture(0, 0, 1, 1, m_PingpongFBO[0], 0, 2);
...
void Renderer::DrawSolidRect(float x, float y, float z, float size, float r, float g, float b, float a)
{
	...
	glEnableVertexAttribArray(attribPosition);
	glBindBuffer(GL_ARRAY_BUFFER, m_VBORect);
	glVertexAttribPointer(attribPosition, 4, GL_FLOAT, GL_FALSE, sizeof(float) * 3, 0);
...
void Renderer::DrawTest()
{
	...
	glUniform1f(uTimeLoc, m_Time);
	int uForceLoc = glGetUniformLocation(m_TestShader, "u_Force");
	glUniform3f(uTimeLoc, 2, 0, 0);
...
void Renderer::DrawParticle()
{
	...
	int uTimeLoc = glGetUniformLocation(shader, "u_Time");
	glUniform1f(uTimeLoc, m_Time);
	int uForce = glGetUniformLocation(shader, "u_Force");
	glUniform3f(uForce, 2, 0, 0);
...
void Renderer::CreateParticles(int particleCounts)
{
	...
	int floatCountsPerVertex = 3 + 1 + 4 + 1 + 3 + 1 + 1 + 1 + 2; // x, y, z, value, r, g, b, a, sTime, vx, vy, vz, tx, ty
	...
	for (int i = 0; i < particleCounts; ++i)
	{
		...
		float centerX = 0.f;// (((float)rand() / (float)RAND_MAX) * 2.f - 1.f) * 0.1f;
		float centerY = 0.f;// (((float)rand() / (float)RAND_MAX) * 2.f - 1.f) * 0.1f;
